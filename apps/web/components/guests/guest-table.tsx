  ...
  if (guests.length === 0) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center">
        ...
        <p className="mt-3 text-sm text-gray-500">No guests yet. Add your first guest to get started.</p>
      </div>
    )
  }
          ...
          <tbody className="divide-y divide-gray-100">
            {guests.map((guest) => (
              <tr
                ...
                className="cursor-pointer transition-colors hover:bg-wedding-50/30"
              >
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    {guest.isVip && (
                      <span className="rounded-full bg-amber-50 px-1.5 py-0.5 text-xs font-medium text-amber-700">VIP</span>
                    )}
                    {guest.isChild && (
                      <span className="rounded-full bg-purple-50 px-1.5 py-0.5 text-xs font-medium text-purple-700">Child</span>
                    )}
                  </div>
                ...
                <td className="px-4 py-3">
                  <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium capitalize ${RSVP_BADGE[guest.rsvpStatus] ?? ''}`}>
                    {guest.rsvpStatus}
                  </span>
                </td>
                <td className="px-4 py-3 text-gray-600">
