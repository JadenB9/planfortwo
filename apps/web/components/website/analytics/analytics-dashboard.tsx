  ...
  if (!canAccess) {
    return (
      <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center">
        ...
        <p className="mt-1 text-xs text-gray-500">Upgrade to Full Access to see visitor analytics.</p>
      </div>
    )
  }

  if (!analytics) {
    return <p className="text-sm text-gray-500">No analytics data yet. Publish your website to start tracking visitors.</p>
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold">
              {analytics.viewsBySection[0]?.section ?? '—'}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      {analytics.viewsByDay.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-sm">Views by Day</CardTitle></CardHeader>
          <CardContent>
              ...
              {analytics.viewsByDay.slice(-14).map((d) => {
                ...
                return (
                  <div key={d.date} className="flex flex-1 flex-col items-center gap-1">
                    <div
                      className="w-full rounded-t bg-wedding-500"
