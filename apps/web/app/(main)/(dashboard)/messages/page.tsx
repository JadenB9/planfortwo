...
function MessagesPageInner() {
  ...
  const loadEntries = useCallback(async () => {
    ...
    try {
      ...
      if (!token) return
      const { data } = await api.guestbook.list(weddingId, token)
      setEntries(data)
    } catch {
    ...
    >
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Review and manage messages from your wedding website guests
          {pendingCount > 0 && (
        ...
        <Card>
          <CardContent className="py-16 text-center">
            <div className="bg-wedding-50 mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full">
              <MessageSquare className="text-wedding-600 h-8 w-8" />
            </div>
            <h2 className="font-serif text-xl font-semibold text-gray-900">No Messages Yet</h2>
            <p className="mx-auto mt-2 max-w-md text-sm text-gray-600">
              When guests leave messages in your wedding website guestbook, they will appear here
        ...
        <Card>
          <CardContent className="py-8 text-center">
            <p className="text-sm text-gray-500">
              No {filter === 'pending' ? 'pending' : 'approved'} messages.
            </p>
          </CardContent>
        </Card>
      ) : (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="mb-2 flex items-center gap-3">
                          <div className="bg-wedding-100 text-wedding-700 flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-semibold">
                        ...
                        <p className="ml-12 whitespace-pre-wrap text-sm leading-relaxed text-gray-700">
                          {entry.message}
                        </p>
                      </div>
      ...
      >
        <DialogContent>
          ...
          <p className="text-sm text-gray-600">
            Are you sure you want to delete this guestbook message? This action cannot be undone.
          </p>
          <DialogFooter>
