...
export default function CommunicationPage() {
  ...
  const [form, setForm] = useState({ subject: '', body: '', templateType: 'custom', scheduledAt: '' })
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (!token) return
      if (editingCampaign) {
        await api.emailCampaigns.update(editingCampaign.id, weddingId, {
    ...
    >
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Create and manage email campaigns for your guests.</p>
        </div>
        <Button onClick={() => {
          setEditingCampaign(null)
          setForm({ subject: '', body: '', templateType: 'custom', scheduledAt: '' })
        ...
        <Card>
          <CardContent className="py-16 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-wedding-50">
              <svg className="h-8 w-8 text-wedding-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
            </div>
            <h2 className="font-serif text-xl font-semibold text-gray-900">No Campaigns Yet</h2>
            <p className="mx-auto mt-2 max-w-md text-sm text-gray-600">Send save-the-dates, invitations, reminders, and thank-you notes to your guests.</p>
          </CardContent>
        </Card>
      ) : (
              ...
              <Card>
                <CardContent className="flex items-center justify-between py-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3">
                      ...
                      {campaign.templateType && (
                        <Badge variant="outline" className="text-xs">{TEMPLATE_LABELS[campaign.templateType] ?? campaign.templateType}</Badge>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-700" onClick={() => handleDelete(campaign.id)}>
                      Delete
                    </Button>
                  </div>
                </CardContent>
      ...
      <Dialog open={showComposeDialog} onOpenChange={setShowComposeDialog}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>Subject</Label>
              <Input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="e.g. You're Invited!" />
            </div>
            <div>
              <Label>Body</Label>
              <Textarea value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} rows={8} placeholder="Write your message..." />
            </div>
            <div>
