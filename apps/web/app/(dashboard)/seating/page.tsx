...
export default function SeatingPage() {
  ...
  const loadChartDetail = useCallback(async (chartId: string) => {
    if (!weddingId) return
    try {
      const token = await getToken()
      if (!token) return
  ...
  const handleAddTable = async () => {
    ...
    try {
      const token = await getToken()
      if (!token) return
      await api.seatingCharts.addTable(selectedChart.id, {
    ...
    >
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Plan your table arrangements and guest seating.</p>
        </div>
      ...
      {selectedChart ? (
        <div>
          <div className="mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-serif text-xl font-semibold text-gray-900">{selectedChart.name}</h2>
          ...
          {selectedChart.tables.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">Add tables to start arranging your seating.</p>
              </CardContent>
            </Card>
          ) : (
            <motion.div
                  ...
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <div className="mt-1 h-2 w-full overflow-hidden rounded-full bg-gray-100">
                        <div
                          className="h-full rounded-full bg-wedding-500 transition-all"
                          style={{ width: `${Math.min(100, (table.assignments.length / table.capacity) * 100)}%` }}
                        />
                      </div>
                    </CardContent>
                  </Card>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <Button className="mt-6" onClick={() => setShowNewChart(true)}>Create Your First Chart</Button>
          </CardContent>
        </Card>
      ) : (
        <motion.div
        ...
        >
          {charts.map((chart) => (
            <motion.div key={chart.id} variants={fadeInUp}>
              <Card className="cursor-pointer transition-shadow hover:shadow-md" onClick={() => loadChartDetail(chart.id)}>
                <CardHeader className="flex flex-row items-center justify-between pb-2">
