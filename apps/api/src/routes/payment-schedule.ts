...
import { z } from 'zod'
import { createPaymentScheduleSchema, updatePaymentScheduleSchema } from '@planfortwo/validators'
...
paymentScheduleRoute.put(
  ...
  async (c) => {
    ...
    if (!updated) {
      return c.json({ error: 'Payment not found', code: 'PAYMENT_NOT_FOUND', statusCode: 404 }, 404)
    }
