...
describe('Seating Chart Routes', () => {
  ...
  describe('GET /seating-charts', () => {
    it('should list charts for a wedding', async () => {
      const mockCharts = [
        { id: CHART_ID, weddingId: WEDDING_ID, name: 'Reception', eventName: null, createdAt: new Date(), updatedAt: new Date() },
      ]
      mockedService.listCharts.mockResolvedValue(mockCharts as never)

      const app = createApp()
      const res = await app.request(`/seating-charts?weddingId=${WEDDING_ID}`, { method: 'GET', headers: authHeaders() })
  ...
  describe('POST /seating-charts', () => {
    it('should create a seating chart', async () => {
      mockedService.createChart.mockResolvedValue({
        id: CHART_ID, weddingId: WEDDING_ID, name: 'Reception',
        eventName: null, canvasData: null, width: 1200, height: 800,
        floorPlanUrl: null, createdAt: new Date(), updatedAt: new Date(),
      } as never)
  ...
  describe('POST /seating-charts/:id/tables', () => {
    it('should add a table', async () => {
      mockedService.addTable.mockResolvedValue({
        id: TABLE_ID, chartId: CHART_ID, label: 'Table 1',
        tableType: 'round', capacity: 8, posX: 100, posY: 100,
        rotation: 0, width: 100, height: 100, createdAt: new Date(),
      } as never)
  ...
  describe('POST /seating-charts/assignments', () => {
    it('should assign a guest to a table', async () => {
      ...
      mockedService.assignGuest.mockResolvedValue({
        ...
        tableId: TABLE_ID, guestId, seatNumber: 1, createdAt: new Date(),
      } as never)
  ...
  describe('GET /seating-charts/:id/conflicts', () => {
    it('should return conflicts and capacity warnings', async () => {
      mockedService.checkConflicts.mockResolvedValue({
        conflicts: [], capacityWarnings: [],
      } as never)

      const app = createApp()
      const res = await app.request(`/seating-charts/${CHART_ID}/conflicts?weddingId=${WEDDING_ID}`, {
  ...
  describe('POST /seating-charts/:id/clone', () => {
    it('should clone a chart', async () => {
      mockedService.cloneChart.mockResolvedValue({
        ...
        name: 'Reception Copy', eventName: null, canvasData: null,
        floorPlanUrl: null, width: 1200, height: 800,
  ...
  describe('POST /seating-charts/relationships', () => {
    it('should create a guest relationship', async () => {
      ...
      mockedService.createRelationship.mockResolvedValue({
        ...
        weddingId: WEDDING_ID, guestAId: guestA, guestBId: guestB,
        relationshipType: 'keep_apart', notes: null, createdAt: new Date(),
      } as never)
