...
describe('Ceremony Routes', () => {
  ...
  describe('GET /ceremony/outlines', () => {
    it('should list ceremony outlines', async () => {
      const mockOutlines = [
        { id: OUTLINE_ID, weddingId: WEDDING_ID, moment: 'processional', title: 'Bride entrance', description: null, duration: 5, sortOrder: 0, createdAt: new Date(), updatedAt: new Date() },
      ]
      mockedService.listOutlines.mockResolvedValue(mockOutlines as never)

      const app = createApp()
      const res = await app.request(`/ceremony/outlines?weddingId=${WEDDING_ID}`, { method: 'GET', headers: authHeaders() })
  ...
  describe('POST /ceremony/outlines', () => {
    it('should create a ceremony outline', async () => {
      mockedService.createOutline.mockResolvedValue({
        id: OUTLINE_ID, weddingId: WEDDING_ID, moment: 'vows', title: 'Exchange vows',
        description: null, duration: 10, sortOrder: 0, createdAt: new Date(), updatedAt: new Date(),
      } as never)
  ...
  describe('PUT /ceremony/outlines/:id', () => {
    it('should update a ceremony outline', async () => {
      mockedService.updateOutline.mockResolvedValue({
        id: OUTLINE_ID, weddingId: WEDDING_ID, moment: 'vows', title: 'Updated',
        description: null, duration: 15, sortOrder: 0, createdAt: new Date(), updatedAt: new Date(),
      } as never)
  ...
  describe('GET /ceremony/vows', () => {
    it('should get vow workspace for current user', async () => {
      mockedService.getVow.mockResolvedValue({
        ...
        userId: 'db-user-id', content: 'My dearest...', isRevealed: false,
        createdAt: new Date(), updatedAt: new Date(),
      } as never)

      const app = createApp()
      const res = await app.request(`/ceremony/vows?weddingId=${WEDDING_ID}`, { method: 'GET', headers: authHeaders() })
      expect(res.status).toBe(200)
      const body = await res.json()
  ...
  describe('PUT /ceremony/vows', () => {
    it('should upsert vow content', async () => {
      mockedService.upsertVow.mockResolvedValue({
        id: 'e0000000-0000-0000-0000-000000000001', weddingId: WEDDING_ID,
        userId: 'db-user-id', content: 'Updated vows', isRevealed: false,
        createdAt: new Date(), updatedAt: new Date(),
      } as never)
  ...
  describe('Processional', () => {
    it('should list processional entries', async () => {
      mockedService.listProcessional.mockResolvedValue([
        { id: 'f0000000-0000-0000-0000-000000000001', weddingId: WEDDING_ID, name: 'Best Man', role: 'Best Man', sortOrder: 0, createdAt: new Date() },
      ] as never)

      const app = createApp()
      const res = await app.request(`/ceremony/processional?weddingId=${WEDDING_ID}`, { method: 'GET', headers: authHeaders() })
      expect(res.status).toBe(200)
      const body = await res.json()
      expect(body.data).toHaveLength(1)
    })
    ...
    it('should create a processional entry', async () => {
      mockedService.createProcessionalEntry.mockResolvedValue({
        ...
        name: 'Flower Girl', role: 'Flower Girl', sortOrder: 0, createdAt: new Date(),
      } as never)
    ...
    it('should update a processional entry', async () => {
      mockedService.updateProcessionalEntry.mockResolvedValue({
        id: 'f0000000-0000-0000-0000-000000000001', weddingId: WEDDING_ID,
        name: 'Updated', role: null, sortOrder: 1, createdAt: new Date(),
      } as never)

      const app = createApp()
      const res = await app.request(`/ceremony/processional/f0000000-0000-0000-0000-000000000001?weddingId=${WEDDING_ID}`, {
        method: 'PUT', headers: authHeaders(),
    ...
    it('should delete a processional entry', async () => {
      mockedService.deleteProcessionalEntry.mockResolvedValue({ id: 'f0000000-0000-0000-0000-000000000001' } as never)
      const app = createApp()
      const res = await app.request(`/ceremony/processional/f0000000-0000-0000-0000-000000000001?weddingId=${WEDDING_ID}`, {
        method: 'DELETE', headers: authHeaders(),
      })
