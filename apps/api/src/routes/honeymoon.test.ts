  ...
  describe('GET /honeymoon', () => {
    it('should list honeymoon plans', async () => {
      mockedService.listPlans.mockResolvedValue([
        { id: PLAN_ID, weddingId: WEDDING_ID, destination: 'Maldives', startDate: null, endDate: null, budget: 5000, notes: null, documents: [], packingList: [], createdAt: new Date(), updatedAt: new Date() },
      ] as never)

      const app = createApp()
      const res = await app.request(`/honeymoon?weddingId=${WEDDING_ID}`, { method: 'GET', headers: authHeaders() })
  ...
  describe('POST /honeymoon', () => {
    it('should create a honeymoon plan', async () => {
      mockedService.createPlan.mockResolvedValue({
        id: PLAN_ID, weddingId: WEDDING_ID, destination: 'Bali',
        startDate: null, endDate: null, budget: 3000, notes: null,
        documents: [], packingList: [], createdAt: new Date(), updatedAt: new Date(),
      } as never)
  ...
  describe('GET /honeymoon/:id', () => {
    it('should get plan with activities', async () => {
      mockedService.getPlan.mockResolvedValue({
        id: PLAN_ID, weddingId: WEDDING_ID, destination: 'Maldives',
        startDate: null, endDate: null, budget: 5000, notes: null,
        documents: [], packingList: [], createdAt: new Date(), updatedAt: new Date(),
        activities: [{ id: ACTIVITY_ID, planId: PLAN_ID, dayNumber: 1, title: 'Snorkeling', description: null, location: 'Resort reef', startTime: '10:00', endTime: '12:00', cost: 50, sortOrder: 0, createdAt: new Date() }],
      } as never)

      const app = createApp()
      const res = await app.request(`/honeymoon/${PLAN_ID}?weddingId=${WEDDING_ID}`, { method: 'GET', headers: authHeaders() })
      expect(res.status).toBe(200)
      const body = await res.json()
  ...
  describe('PUT /honeymoon/:id', () => {
    it('should update a plan', async () => {
      mockedService.updatePlan.mockResolvedValue({
        id: PLAN_ID, weddingId: WEDDING_ID, destination: 'Updated',
        startDate: null, endDate: null, budget: 6000, notes: null,
        documents: [], packingList: [], createdAt: new Date(), updatedAt: new Date(),
      } as never)
  ...
  describe('POST /honeymoon/:id/activities', () => {
    it('should add an activity', async () => {
      mockedService.addActivity.mockResolvedValue({
        id: ACTIVITY_ID, planId: PLAN_ID, dayNumber: 2, title: 'Spa Day',
        description: null, location: null, startTime: null, endTime: null,
        cost: 200, sortOrder: 0, createdAt: new Date(),
      } as never)
  ...
  describe('PUT /honeymoon/activities/:activityId', () => {
    it('should update an activity', async () => {
      mockedService.updateActivity.mockResolvedValue({
        id: ACTIVITY_ID, planId: PLAN_ID, dayNumber: 2, title: 'Updated Spa',
        description: null, location: null, startTime: null, endTime: null,
        cost: 250, sortOrder: 0, createdAt: new Date(),
      } as never)

      const app = createApp()
      const res = await app.request(`/honeymoon/activities/${ACTIVITY_ID}?weddingId=${WEDDING_ID}`, {
        method: 'PUT', headers: authHeaders(),
