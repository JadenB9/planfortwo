...
import type {
  CreateThankYouNoteInput,
  UpdateThankYouNoteInput,
...
export const thankYouService = {
  async list(weddingId: string) {
    return db
      .select()
      .from(thankYouNotes)
      .where(eq(thankYouNotes.weddingId, weddingId))
      .orderBy(asc(thankYouNotes.createdAt))
  },
  ...
  async getById(noteId: string, weddingId: string) {
    const [note] = await db
      .select()
      .from(thankYouNotes)
      .where(and(eq(thankYouNotes.id, noteId), eq(thankYouNotes.weddingId, weddingId)))
    return note ?? null
  },

  async create(data: CreateThankYouNoteInput) {
    const [note] = await db
      .insert(thankYouNotes)
      .values({
...
export const nameChangeService = {
  async list(weddingId: string) {
    return db
      .select()
      .from(nameChangeTasks)
      .where(eq(nameChangeTasks.weddingId, weddingId))
      .orderBy(asc(nameChangeTasks.sortOrder))
  },
  ...
  async toggleComplete(taskId: string, weddingId: string) {
    const [existing] = await db
      .select()
      .from(nameChangeTasks)
      .where(and(eq(nameChangeTasks.id, taskId), eq(nameChangeTasks.weddingId, weddingId)))
    if (!existing) return null

    const [updated] = await db
      .update(nameChangeTasks)
      .set({
        isCompleted: !existing.isCompleted,
        completedAt: existing.isCompleted ? null : new Date(),
      })
      .where(eq(nameChangeTasks.id, taskId))
      .returning()
    return updated ?? null
  },
...
export const vendorReviewService = {
  async list(weddingId: string) {
    return db
      .select()
      .from(vendorReviews)
      .where(eq(vendorReviews.weddingId, weddingId))
      .orderBy(asc(vendorReviews.createdAt))
  },
  ...
  async delete(reviewId: string, weddingId: string) {
    const [deleted] = await db
      ...
      .where(and(eq(vendorReviews.id, reviewId), eq(vendorReviews.weddingId, weddingId)))
      .returning()
    return !!deleted
  },
}
...
export const notificationPrefService = {
    ...
    const [prefs] = await db
      ...
      .where(
        and(
          ...
          eq(notificationPreferences.weddingId, weddingId),
        ),
      )
    return prefs ?? null
  },
  ...
  async upsert(userId: string, weddingId: string, data: UpdateNotificationPreferencesInput) {
    ...
    if (existing) {
      ...
      if (data.emailPaymentReminder !== undefined)
        updateData.emailPaymentReminder = data.emailPaymentReminder
      if (data.emailTaskDue !== undefined) updateData.emailTaskDue = data.emailTaskDue
      if (data.emailWeeklySummary !== undefined)
        updateData.emailWeeklySummary = data.emailWeeklySummary
